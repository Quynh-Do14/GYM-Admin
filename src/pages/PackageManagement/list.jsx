...
const ListPackageManagement = () => {
    ...
    const onGetPackageAsync = async ({ name = "", size = pageSize, page = currentPage, startDate = "", endDate = "" }) => {
        const param = {
            page: page - 1,
            size: size,
            name: name,
    ...
    const onSearch = async (name = "", size = pageSize, page = 1, startDate = "", endDate = "") => {
        await onGetPackageAsync({ name: name, size: size, page: page, startDate: startDate, endDate: endDate });
    };
    ...
    const onDeleteEquipType = async () => {
        setIsDeleteModal(false);
        try {
            await packageService.deletePackages(
                idSelected,
                setLoading
            ).then((res) => {
                if (res) {
    ...
    const onNavigate = (id) => {
        navigate(`${(ROUTE_PATH.VIEW_PACKAGE).replace(`${Constants.UseParams.Id}`, "")}${id}`);
    }
    return (
        <MainLayout breadcrumb={"Quản lý gói thành viên"} title={"Danh sách gói thành viên"} redirect={""}>
            <div className='flex flex-col header-page'>
                <Row className='filter-page mb-2 py-2-5' gutter={[10, 10]} justify={"space-between"} align={"middle"}>
                    ...
                    <Col>
                        <ButtonCommon icon={<PlusOutlined />} classColor="orange" onClick={() => navigate(ROUTE_PATH.ADD_PACKAGE)} >Thêm mới</ButtonCommon>
                    </Col>
                </Row>
            </div>
            ...
            <DialogConfirmCommon
                message={"Bạn có muốn xóa gói thành viên này ra khỏi hệ thống"}
                titleCancel={"Bỏ qua"}
                titleOk={"Xóa gói thành viên"}
...
export default ListPackageManagement