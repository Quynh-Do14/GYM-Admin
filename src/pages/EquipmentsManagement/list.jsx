...
const ListEquipmentManagement = () => {
    ...
    const onGetEquipmentAsync = async ({ name = "", size = pageSize, page = currentPage, startDate = "", endDate = "" }) => {
        const param = {
            page: page - 1,
            size: size,
            name: name,
    ...
    const onSearch = async (name = "", size = pageSize, page = 1, startDate = "", endDate = "") => {
        await onGetEquipmentAsync({ name: name, size: size, page: page, startDate: startDate, endDate: endDate });
    };
    ...
    const onNavigate = (id) => {
        navigate(`${(ROUTE_PATH.VIEW_EQUIPMENT).replace(`${Constants.UseParams.Id}`, "")}${id}`);
    }
    return (
        <MainLayout breadcrumb={"Quản lý dụng cụ"} title={"Danh sách dụng cụ"} redirect={""}>
            <div className='flex flex-col header-page'>
                <Row className='filter-page mb-2 py-2-5' gutter={[10, 10]} justify={"space-between"} align={"middle"}>
                    ...
                    <Col>
                        <ButtonCommon icon={<PlusOutlined />} classColor="orange" onClick={() => navigate(ROUTE_PATH.ADD_EQUIPMENT)} >Thêm mới</ButtonCommon>
                    </Col>
                </Row>
            </div>
            ...
            <DialogConfirmCommon
                message={"Bạn có muốn xóa dụng cụ này ra khỏi hệ thống"}
                titleCancel={"Bỏ qua"}
                titleOk={"Xóa dụng cụ"}
                visible={isDeleteModal}
                handleCancel={onCloseModalDelete}
                handleOk={onDeleteEquipment}
                title={"Xác nhận"}
            />
...
export default ListEquipmentManagement