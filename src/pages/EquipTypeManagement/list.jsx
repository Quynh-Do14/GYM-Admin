...
const ListEquipTypeManagement = () => {
    ...
    const onGetEquipTypeAsync = async ({ name = "", size = pageSize, page = currentPage, startDate = "", endDate = "" }) => {
        const param = {
            page: page - 1,
            size: size,
            name: name,
    ...
    const onSearch = async (name = "", size = pageSize, page = 1, startDate = "", endDate = "") => {
        await onGetEquipTypeAsync({ name: name, size: size, page: page, startDate: startDate, endDate: endDate });
    };
    ...
    const onDeleteEquipType = async () => {
        setIsDeleteModal(false);
        try {
            await equipTypeService.deleteEquipType(
                idSelected,
                setLoading
            ).then((res) => {
                if (res) {
    ...
    const onNavigate = (id) => {
        navigate(`${(ROUTE_PATH.VIEW_EQUIP_TYPE).replace(`${Constants.UseParams.Id}`, "")}${id}`);
    }
    return (
        <MainLayout breadcrumb={"Quản lý loại thiết bị"} title={"Danh sách loại thiết bị"} redirect={""}>
            <div className='flex flex-col header-page'>
                <Row className='filter-page mb-2 py-2-5' gutter={[10, 10]} justify={"space-between"} align={"middle"}>
                    ...
                    <Col>
                        <ButtonCommon icon={<PlusOutlined />} classColor="orange" onClick={() => navigate(ROUTE_PATH.ADD_EQUIP_TYPE)} >Thêm mới</ButtonCommon>
                    </Col>
                </Row>
            </div>
            <div className='flex-1 overflow-auto bg-[#FFFFFF] content-page'>
                <Table
                    dataSource={listEquipType}
                    pagination={false}
            ...
            <DialogConfirmCommon
                message={"Bạn có muốn xóa loại thiết bị này ra khỏi hệ thống"}
                titleCancel={"Bỏ qua"}
                titleOk={"Xóa loại thiết bị"}
                visible={isDeleteModal}
                handleCancel={onCloseModalDelete}
                handleOk={onDeleteEquipType}
                title={"Xác nhận"}
            />
...
export default ListEquipTypeManagement