...
import userService from '../../infrastructure/repositories/user-management/service/user.service'
...
const ListShiftManagement = () => {
    ...
    const onSearch = async (name = "", size = pageSize, page = 1, startDate = "", endDate = "") => {
        await onGetUserAsync({ name: name, size: size, page: page, startDate: startDate, endDate: endDate });
    };
    ...
    const onNavigate = (id) => {
        navigate(`${(ROUTE_PATH.VIEW_SHIFT).replace(`${Constants.UseParams.Id}`, "")}${id}`);
    }
    return (
        <MainLayout breadcrumb={"Quản lý ca làm việc"} title={"Danh sách ca làm việc"} redirect={""}>
            <div className='flex flex-col header-page'>
                <Row className='filter-page mb-2 py-2-5' gutter={[10, 10]} justify={"space-between"} align={"middle"}>
                    ...
                    <Col>
                        <ButtonCommon icon={<PlusOutlined />} classColor="orange" onClick={() => navigate(ROUTE_PATH.ADD_SHIFT)} >Thêm mới</ButtonCommon>
                    </Col>
                </Row>
            </div>
            ...
            <DialogConfirmCommon
                message={"Bạn có muốn xóa ca làm việc này ra khỏi hệ thống"}
                titleCancel={"Bỏ qua"}
                titleOk={"Xóa ca làm việc"}
...
export default ListShiftManagement